...
const TIME_LIMIT = 30; 
const MIN_WORDS = TIME_LIMIT * 10;

const StatsCard = ({ label, value, color = 'text-slate-700', highlight = false }) => (
...
const WordDisplay = ({ words, currentWordIndex, typedWords, currentInput, getCharStatus }) => (
...
const Results = ({ wpm, accuracy, typedWords, correctChars, totalChars }) => (
  <div className="bg-white rounded-2xl p-8 mb-8 shadow-sm border border-slate-200">
    <h2 className="text-3xl font-bold mb-8 text-center text-slate-800">Test Complete!</h2>
    <div className="grid grid-cols-3 gap-8 mb-8">
      ...
      <div className="text-center">
        ...
        <div className="text-6xl font-bold text-purple-600">{typedWords.length}</div>
      </div>
    </div>
    <div className="text-center text-slate-600 bg-slate-50 rounded-lg py-4">
      <span className="font-medium">Correct:</span> {typedWords.filter(w => w.correct).length} • 
      <span className="font-medium"> Incorrect:</span> {typedWords.filter(w => !w.correct).length} • 
      <span className="font-medium"> Characters:</span> {correctChars}/{totalChars}
    </div>
  </div>
);
...
const Instructions = () => (
  <div className="text-center mt-8 text-slate-600 bg-blue-50 rounded-xl p-6 border border-blue-200">
    ...
    <p className="text-sm mt-2 text-slate-500">The test will automatically start when you begin typing</p>
  </div>
);
...
export default function TypingTest() {
  ...
  const handleInputChange = (e) => {
    ...
    if (value.endsWith(' ')) {
      ...
      if (typedWord) {
        ...
        setTotalChars((prev) => prev + Math.max(typedWord.length, currentWord.length));
  ...
  const getCharStatus = (char, index) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-8">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-600">Test your typing speed and accuracy in {TIME_LIMIT} seconds</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          ...
          <StatsCard label="Accuracy" value={`${calculateAccuracy()}%`} color="text-green-600" />
        </div>
        ...
        {!isFinished && (
          <input
            ...
            placeholder={isActive ? "Type the words above..." : "Click here and start typing to begin..."}
          />
        )}
